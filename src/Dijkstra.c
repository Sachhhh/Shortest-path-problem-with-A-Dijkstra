/* --------------------------------------------------
  Fichier Dijkstra.c pour l'implantation de la stucture sommets 'vertex_t'
...
int Dijkstra(int depart, int arrivee, graph_t g, int affichage){
  ...
  heap_t atraiter = init_atraiter_d(HEAPSIZE); // Tas (ensemble des sommets à traiter)
  tabver_t atteint = init_atteint_d(g.size_vertices); // Tableau de pointeurs sur les sommets déjà atteints
  tabver_t predecesseur = init_atteint_d(g.size_vertices);
